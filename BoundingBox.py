...
def bound_values(image_3d_scan, brightness):
    ...
    brigtness_matrix = np.repeat([np.repeat([RGB_TO_BRIGHTNESS], image_3d_scan.shape[1], axis=0)], image_3d_scan.shape[0], axis=0)
    brightness_image_3d_scan = np.multiply(brigtness_matrix, image_3d_scan)
    brightness_image_2d_scan = np.sum(brightness_image_3d_scan, axis=2)
